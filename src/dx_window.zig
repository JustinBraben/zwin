...
pub const MessageCallback = *const fn (window: *DXWindow, msg: u32, wparam: WPARAM, lparam: LPARAM) ?LRESULT;
...
pub fn init(allocator: std.mem.Allocator, options: CreateOptions) !*DXWindow {
    const window = try allocator.create(DXWindow);
...
pub fn deinit(self: *DXWindow) void {
...
fn registerClass(self: *DXWindow, options: CreateOptions) !void {
    self.class = .{
        .style = .{},
...
/// Create the window
fn createWindow(self: *DXWindow, options: CreateOptions) !void {
...
fn createSwapChain(self: *DXWindow, options: CreateOptions) !void {
    _ = self;
    // Describe swap chain
...
/// Set window title
pub fn setTitle(self: *DXWindow, title: [:0]const u8) void {
